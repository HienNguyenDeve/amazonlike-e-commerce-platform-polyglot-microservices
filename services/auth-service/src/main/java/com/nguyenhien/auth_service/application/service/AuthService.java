...
import com.nguyenhien.auth_service.common.exception.UsernameExistException;
...
public class AuthService implements IAuthService, UserDetailsService {
  ...
  public MessageResponse register(RegisterRequest request) {
    ...
    UserRegisteredEvent event =
        new UserRegisteredEvent(
            UUID.randomUUID(), savedUser.getId(), savedUser.getEmail(), Instant.now());

    authEventProducer.publishUserRegistered(event);

    return MessageResponse.builder().message("Regisgter Successfully").success(true).build();
  }
  ...
  public JwtResponse refreshToken(RefreshTokenRequest request) {
    ...
    List<SimpleGrantedAuthority> authorities =
        List.of(new SimpleGrantedAuthority("ROLE_" + oldRefreshToken.getRole()));
  ...
  public MessageResponse revokeToken(String token, String reason) {
    if (token == null || token.isBlank()) {
      return MessageResponse.builder().message("Refresh token is required").success(false).build();
    }
    boolean result = refreshTokenService.delete(token);
    if (!result) {
      return MessageResponse.builder().message("Token faluire revoke").success(false).build();
    }

    // return
    return MessageResponse.builder().message("Token successfully revoke").success(true).build();
  }
  ...
  public MessageResponse logout(LogoutRequest request) {
    ...
    return MessageResponse.builder().message("Logout successfully").success(true).build();
  }
}
